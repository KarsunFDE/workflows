...
- **`/fde-plan <react|nextjs>`** — load reviewer lenses from persona cards → research → user stories →
  refute-mode persona review → SPEC (component map, NO full code) → critic gate → optional sandbox prototype
  → REPORT (`./fde-modernization-plan.md`). Review tiers: default = 1 agent all lenses + one-shot critic (lean);
  `{rigorous:true}` = one critic PER lens + verdict vote + bounded gap-loop (re-specs until clean, max 2 rounds).
...
### Code findings
- **C1 High — PARTIAL (re-verified 2026-06-22 against the on-disk section set + git log; CORRECTS two earlier
    misreads — the projects were backwards AND foia was wrongly called canonical on a partial string match).**
  The canonical schema requires the FULL section set (Summary · What they can do · **What they cannot do** ·
  Constraints · Impact · **Pain points** · Evidence · Reviewer lens). Tested per project:
  - `contract-payment-flow/` — **CANONICAL, and the ONLY one** (12/12 have all 8 sections incl. `## What they
    cannot do` + `## Pain points`; `## Evidence (file:line)` + `## Reviewer lens`; regenerated by `/fde-personas`
    in git `8a58e63 re-generated new personas`). `personas/README.md` index was missing, now hand-written
    (this branch).
...
- 2026-06-22: branch `fix/inline-skill-rules` — review-driven fixes. **P1:** verified (official docs) that
  workflow subagents do NOT auto-load skill bodies (`agent()` has no `skills` field; only main session preloads),
  so inlined CLUSTER/SCHEMA/COVERAGE rules into `fde-analyze.js` (evidence rules were already inlined) — rigor no
  longer depends on skill auto-load. **P2 (hybrid):** added `{rigorous:true}` to `fde-plan.js` — per-lens critics
  + verdict vote + bounded gap-loop (MAX_GAP_ROUNDS=2); lean single-pass stays default; docs (§8/§10 +
